...
<head>
    ...
    <script type="text/javascript">
        //导出信息
        function daochu() {
            var url = "${contextPath}/lizuoChakanMy?d=1";
            window.location.href=url;
        }
        //删除信息
        function lizuoDel(id) {
            if (confirm('您确定删除吗?')) {
                $.get(basePath + '/lizuoDel/' + id, function (resp) {
                    if (resp.success) {
                        alert("删除成功！");
                        //window.location.href = "${contextPath}/lizuoChankanMy";
        ...
        //跳转到信息修改页
        function lizuoGet(id) {
            window.location.href = "${contextPath}/lizuoGet/" + id;
        }

        //跳转到信息详细页
        function lizuoDetail(id) {
            window.location.href = "${contextPath}/lizuoDetail/" + id;
        }
        ...
        //更新信息
        function lizuoSet(id,property,value) {
            if (confirm('您确定操作吗?')) {
                var param = {};
                param["id"]=id;
                param[property]=value;
                $.post(basePath + '/lizuoSet', param, function(resp) {
                    if (resp.success) {
                        alert("操作成功！");
                        //window.location.href = "${contextPath}/lizuoChankanMy";
                        document.formSearch.submit();
...
<table width="98%" border="0" cellpadding="2" cellspacing="1" bgcolor="#D1DDAA" align="center" style="margin-top:8px"
       bordercolor="#008080">
    <form action="${contextPath}/lizuoChakanMy" name="formSearch" id="formSearch" method="post">
...
<%--        用户:
    ...
    <tr bgcolor="#E7E7E7">
        <td height="14" colspan="100" background="${staticPath}/images/tbg.gif">离座查看</td>
    </tr>
...
<%--                <input type="button" value="修改" onclick="lizuoGet(${info.id})"/>--%>
                <input type="button" value="详情" onclick="lizuoDetail(${info.id})"/>
<%--                <input type="button" value="删除" onclick="lizuoDel(${info.id})"/>--%>
                    <%--sonTable本表子表管理--%>
